...
// after the operation is successful, it dispatches the addPost action 
// with the newly created post as the payload.

export const addPostData = (postData) => async (dispatch) => {
  try {
    const response = await axios.post("http://localhost:5000/posts/", postData);
    const newPost = response.data;
   console.log(newPost);
   // Pass the new post as payload
